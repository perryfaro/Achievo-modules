 ...
 * @author Juriy Zaytsev; kangax [at] gmail [dot] com;
...
Proto.Menu = Class.create({
  initialize : function()
  {
    ...
    this.options.menuItems.each(function(item)
    {
      ...
      }).observe('click', this.onClick.bind(this)).observe('contextmenu',
          Event.stop).update(item.name)))
    }.bind(this));
    $(document.body).insert(this.container.insert(list).observe('contextmenu',
        Event.stop));
    if (this.ie)
    {
    ...
    $$(this.options.selector).invoke('observe',
        Prototype.Browser.Opera ? 'click' : 'contextmenu', function(e)
        {
  ...
  show       : function(e)
  {
    ...
    var x = Event.pointer(e).x, y = Event.pointer(e).y, vpDim = document.viewport
        .getDimensions(), vpOff = document.viewport.getScrollOffsets(), elDim = this.container
        .getDimensions(), elOff = {
      left : ((x + elDim.width + this.options.pageOffset) > vpDim.width
          ? (vpDim.width - elDim.width - this.options.pageOffset)
          : x)
          + 'px',
      top  : ((y - vpOff.top + elDim.height) > vpDim.height
          && (y - vpOff.top) > elDim.height ? (y - elDim.height) : y)
          + 'px'
    };
