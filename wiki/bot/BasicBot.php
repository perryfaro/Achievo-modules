...
$abspath = atkconfig("atktempdir");

// adjust as necessary
define('SITECHARSET','UTF-8');
define('SERVER',atkConfig::get('wiki', 'wiki_server'));
define('PREFIX',atkConfig::get('wiki', 'wiki_prefix')); // no trailing slash. The prefix you use for index.php?title= links (e.g. editing links). Set to '' if you use no prefix other than what's in SERVER.
define('ALTPREFIX',atkConfig::get('wiki', 'wiki_prefix')); // no trailing slash. The prefix on valid links that visitors usually see. Might be the same as PREFIX if you don't use "pretty" links.
...
if (!defined('PASSWORD')){	define('PASSWORD','');} // password in plain text. No md5 or anything.
...
class BasicBot extends Snoopy{
	...
	****************************************/
	...
	function wikiFilter($title,$callback,$summary='',$callbackParams=array()){
		...
		{
			// ignored this situation. IVO HACK
		}
...
function addTemplate($content,$args){
	...
	$match = $template;
	$pipe_pos = strpos($match, "|"); // IVO HACK
	if ($pipe_pos!==false)
	{
		// parameterized template
		$match = substr($match, 0, $pipe_pos); 	
	}
		
	if (inString($content,$match)) // don't add the template twice
		return $content;
	if ($toBottom)
